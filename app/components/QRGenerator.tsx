...
const TRANSLATIONS = {
  "en-US": {
    ...
    "appDescription": "Generate QR codes for URLs, text and contact informations, now with LOGO for your brand presence!",
    "urlTab": "URL",
    "textTab": "Text",
    "contactTab": "Contact",
    "enterUrl": "Enter URL",
    ...
    "urlPlaceholder": "example.com.my or https://example.com.my",
    ...
    "lastName": "Last Name",
    "lastNamePlaceholder": "Ibrahim",
    "phoneNumber": "Phone Number",
    "phonePlaceholder": "+60 12-345 6789",
    "emailAddress": "Email Address",
    "emailPlaceholder": "ahmad.ibrahim@example.com.my",
    "organization": "Organization",
    "organizationPlaceholder": "Sdn Bhd Company",
    "website": "Website",
    "websitePlaceholder": "https://example.com.my",
    ...
    "footerText": "Generate QR codes instantly • No data stored • Free to use • Remixed by Andrew C.",
...
const QRGenerator: React.FC = () => {
  ...
  const createQR = (text: string) => {
    if (!qrContainerRef.current || typeof window === 'undefined') return;
  ...
  const generateFallbackQR = (text: string) => {
    ...
    img.src = `https://chart.googleapis.com/chart?chs=800x800&cht=qr&chl=${encodedData}&choe=UTF-8`;
    img.alt = t('qrCodeAlt');
  ...
  const addLogoToCanvas = (canvas: HTMLCanvasElement) => {
    ...
    img.onload = () => {
      ...
      const logoSizeRatio = sizeMultipliers[logoSize as keyof typeof sizeMultipliers];
...
END:VCARD`;
  ...
  const downloadQRCode = () => {
    ...
    if (canvas) {
      try {
        ...
        downloadFromImage(img as HTMLImageElement);
      }
    } else if (img) {
      downloadFromImage(img as HTMLImageElement);
    }
  ...
  const downloadFromImage = (img: HTMLImageElement) => {
    if (!img) return;
